...
for people in csv.reader(user_file,encoding='utf-8'):
    ...
    if( user == set([]) or user & user_id_set == user_id_set ):# 判斷此user是否在限制名單內,若名單為空則視為不限制user
        ...
        table = read_table(user_path+"/activities_table.csv") #讀取使用者 activities_table 中的資料,並轉換為 set 形式回傳

        for activities in activities_mes:
            id = activities["id"]
            id_set = set([str(id)]) #轉換id成set形式
            if(table & id_set == set([])): # 判斷此筆activities資料是否已經存在
